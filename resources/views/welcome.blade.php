    ...
    <div class="max-w-[1440px] mx-auto py-12 px-6 lg:px-12" x-data="{ show: false }"
        ...
        <div x-show="show" x-transition:enter="transition ease-out duration-1000"
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 items-center gap-10 lg:gap-6">
                ...
                <!-- Right: Market Watch -->
                <div class="order-3 lg:order-3 flex flex-col items-center lg:items-end gap-3 ">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-12 mb-24">
                ...
                <div class="lg:col-span-8 group cursor-pointer">
                    <div class="flex flex-col gap-5">
                        <div
                            class="aspect-[16/9] w-full overflow-hidden rounded-md bg-gray-100 shadow-xl shadow-blue-900/5">
                        ...
                        <div class="flex flex-col gap-3">
                            <div class="flex items-center gap-3">
                                <span
                                    class="bg-blue-700 text-white text-[9px] font-black uppercase tracking-[0.2em] px-2.5 py-0.5 rounded shadow-lg shadow-blue-500/30">
                                    {{ $featured->category->name ?? 'Update' }}
                                </span>
                                <span
                                    class="text-[9px] font-bold text-gray-500 uppercase tracking-widest">{{ $featured->created_at->diffForHumans() }}</span>
                            </div>
                            <h2
                                class="text-3xl lg:text-5xl font-black text-gray-900 leading-[1.1] tracking-tighter group-hover:text-blue-700 transition-colors">
                            ...
                            <div class="flex items-center justify-between pt-5 border-t border-gray-100 mt-1">
                                <div class="flex items-center gap-3">
                                    <div
                                        class="w-8 h-8 rounded-full bg-blue-100 flex items-center justify-center text-blue-700 font-extrabold text-xs ring-2 ring-white shadow-sm">
                                        {{ substr($featured->user->name, 0, 1) }}
                                    </div>
                                    <div class="flex flex-col">
                                        <span
                                            class="text-xs font-black text-gray-900 leading-tight">{{ $featured->user->name }}</span>
                                        <span
                                            class="text-[9px] font-bold text-gray-400 uppercase tracking-widest leading-none">Reporter</span>
                                    </div>
                                </div>
                ...
                <div class="lg:col-span-4 border-l border-gray-100 lg:pl-10 flex flex-col gap-8">
                    <h3
                        class="text-[10px] font-black uppercase tracking-[0.4em] text-gray-900 border-b-2 border-gray-900 pb-3">
                        Most Popular</h3>
                        ...
                        @foreach($articles->slice(1, 4) as $index => $side)
                            <div class="flex flex-col gap-3 group">
                                <div class="flex items-start gap-3">
                                    <span
                                        class="text-4xl font-black text-gray-100 group-hover:text-blue-600 transition-colors leading-[0.8]">0{{ $index + 1 }}</span>
                                    <div class="flex flex-col gap-1.5">
                                        <span
                                            class="text-[9px] font-black uppercase tracking-widest text-blue-700">{{ $side->category->name ?? 'Focus' }}</span>
                                        <h4
                                            class="text-lg font-extrabold text-gray-900 leading-tight group-hover:text-blue-700 transition-colors tracking-tight">
                                ...
                                <div class="flex items-center justify-between pt-3 border-t border-gray-50 border-dashed">
                                    <span
                                        class="text-[9px] font-bold text-gray-500 uppercase tracking-widest">{{ $side->created_at->translatedFormat('d M') }}</span>
            ...
            <div class="mb-24">
                <div class="flex items-baseline justify-between mb-8 border-b-2 border-gray-900 pb-3">
                    ...
                    <div class="flex items-center border border-gray-100 rounded-lg p-0.5 bg-gray-50">
                        <button
                            class="text-[9px] font-black uppercase tracking-widest px-3 py-1.5 bg-white rounded shadow-sm text-blue-700">All
                            Posts</button>
                        <button
                            class="text-[9px] font-black uppercase tracking-widest px-3 py-1.5 text-gray-400 hover:text-gray-900">Analysis</button>
                        <button
                            class="text-[9px] font-black uppercase tracking-widest px-3 py-1.5 text-gray-400 hover:text-gray-900">Opinion</button>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-y-12 border-b border-gray-100 pb-12">
                    @foreach($articles->skip(5)->take(8) as $gridItem)
                        <div
                            class="flex flex-col gap-5 md:px-4 md:first:pl-0 md:last:pr-0 md:border-r border-gray-100 md:last:border-0 group">
                            ...
                            <div class="flex flex-col gap-2">
                                <span
                                    class="text-[9px] font-black uppercase tracking-[0.2em] text-blue-700">{{ $gridItem->category->name ?? 'News' }}</span>
                                <h4
                                    class="text-base lg:text-lg font-extrabold text-gray-900 leading-tight tracking-tight group-hover:text-blue-700 transition-colors line-clamp-3">
                                ...
                                <div class="flex items-center justify-between pt-4 border-t border-gray-100 mt-1">
                                    <span
                                        class="text-[9px] font-black text-gray-400 uppercase tracking-widest">{{ $gridItem->created_at->translatedFormat('d M Y') }}</span>
                ...
                @foreach($categorized as $categoryName => $catArticles)
                    <section class="border-t-2 border-gray-900 pt-10">
                        <div class="flex items-center justify-between mb-12 px-2">
                            <h3
                                class="text-3xl lg:text-4xl font-black text-gray-900 tracking-tighter uppercase italic leading-none">
                                {{ $categoryName }}</h3>
                            <a href="#"
                                class="text-[9px] font-black text-gray-900 uppercase tracking-[0.3em] border-[1.5px] border-gray-900 px-4 py-1.5 hover:bg-gray-900 hover:text-white transition-all">View
                                {{ $categoryName }}</a>
                        </div>
                        ...
                        <div class="grid grid-cols-1 lg:grid-cols-12 gap-10">
                            ...
                            <div class="lg:col-span-7 lg:border-r border-gray-100 lg:pr-10 group">
                                ...
                                <div class="flex flex-col gap-6">
                                    ...
                                    <div class="flex flex-col gap-3">
                                        <h4
                                            class="text-2xl lg:text-3xl font-extrabold text-gray-900 leading-tight tracking-tighter group-hover:text-blue-700 transition-colors">
                                        ...
                                        <div class="flex items-center justify-between pt-4 border-t border-gray-100">
                                            <span
                                                class="text-[9px] font-black text-gray-500 uppercase tracking-widest">{{ $bigCat->created_at->translatedFormat('d M Y') }}</span>
                            ...
                            <div class="lg:col-span-5 flex flex-col gap-6 lg:mt-0 mt-10">
                                @foreach($catArticles->skip(1)->take(3) as $catItem)
                                    <div class="flex gap-4 md:gap-5 group pb-6 border-b border-gray-50 last:border-0 last:pb-0">
                                        <div
                                            class="flex-shrink-0 w-20 h-20 md:w-28 md:h-28 rounded-md overflow-hidden bg-gray-50 border border-gray-200">
                                        ...
                                        <div class="flex flex-col justify-center gap-1.5 flex-1 min-w-0">
                                            <h4
                                                class="text-base font-extrabold text-gray-900 leading-tight group-hover:text-blue-700 transition-colors font-sans tracking-tight line-clamp-2">
                                                <a href="{{ route('articles.show', $catItem->id) }}">{{ $catItem->title }}</a>
                                            </h4>
                                            <p
                                                class="text-[10px] text-gray-600 font-bold line-clamp-1 md:line-clamp-2 leading-relaxed">
                                                {{ Str::limit(strip_tags($catItem->content), 70, '...') }}
                                            </p>
                                            <div class="flex items-center gap-3 mt-0.5">
                                                <span
                                                    class="text-[9px] font-black text-gray-400 uppercase tracking-widest">{{ $catItem->created_at->translatedFormat('d M') }}</span>
