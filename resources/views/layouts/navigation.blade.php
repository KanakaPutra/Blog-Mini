                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex items-center">
                    ...
                    <div class="relative" x-data="{ hover: false }" @mouseenter="hover = true" @mouseleave="hover = false">
                        <button 
                            ...
                            {{ __('Index') }}
                            <svg :class="{ 'rotate-180': openIndex }" class="ms-1 h-4 w-4 transform transition-transform duration-200"
                ...
                @if (Auth::check())
                    <div class="relative">
                        <button @click="openProfile = !openProfile"
                            ...
                            <svg :class="{ 'rotate-180': openProfile }" class="ms-1 h-4 w-4 transform transition-transform duration-200"
                                xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7" />
                            </svg>
                        </button>
    ...
    <div :class="{ 'block': open, 'hidden': !open }" class="hidden sm:hidden" x-cloak>
            ...
            @auth
                ...
                @if(auth()->user()->is_admin == 2)
                    <div class="border-t border-gray-200 mt-2 px-4 pt-2">
                        ...
                        <a href="{{ route('superadmin.users') }}" class="block px-3 py-1.5 text-sm text-gray-700 hover:bg-gray-100">
                            {{ __('Manage Users') }}
                        </a>
                        <a href="{{ route('superadmin.settings') }}" class="block px-3 py-1.5 text-sm text-gray-700 hover:bg-gray-100">
                            {{ __('Settings') }}
                        </a>
                    </div>
                @endif
        ...
        @else
            <div class="pt-4 pb-3 border-t border-gray-200 px-4">
                ...
                <a href="{{ route('register') }}" class="block text-gray-600 hover:text-gray-800 mt-2">{{ __('Register') }}</a>
            </div>
        @endif
    </div>
</nav>
